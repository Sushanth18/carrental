...
export class ReservationConfirm extends React.Component {
    ...
    .then(response=>{
      response.json().then(json=>{
		...
		const url2=`http://localhost:8080/CarRental/userdata/all?username=${sessionStorage.getItem("username")&&sessionStorage.getItem("username")!="null"?sessionStorage.getItem("username"):""}`;
