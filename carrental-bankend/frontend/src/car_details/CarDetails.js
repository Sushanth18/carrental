...
export class CarDetails extends React.Component {
	...
	componentDidMount(){
		...
		const url2=`http://localhost:8080/CarRental/userdata/isauthenticated?username=${sessionStorage.getItem("username")&&sessionStorage.getItem("username")!="null"?sessionStorage.getItem("username"):""}`;
		fetch(url2)
		.then(response=>{
			response.json().then(json=>{
		...
		const url3=`http://localhost:8080/CarRental/userdata/all?username=${sessionStorage.getItem("username")&&sessionStorage.getItem("username")!="null"?sessionStorage.getItem("username"):""}`;
		fetch(url3)
		.then(response => response.json())
