...
public class VehicleController {
	...
	public Page<Vehicle> getVehicleList(@RequestParam(value = "page") int page,
		...
		return vehicleService.getVehiclesForPage(PageRequest.of(page, number), userId);
	}
...
//		+ FilenameUtils.getExtension(image.getOriginalFilename());
	...
	public List<UserDetail> getVehicleUser(@PathVariable Long id) {
		List<UserDetail> details = new ArrayList<>();
		Vehicle vehicle = vehicleService.getVehicleById(id);
		User user = userService.getUserById(vehicle.getUserId());
		details.add(getUserDetail(user));
		return details;
	}
	...
	@RequestMapping(value = { "/customer-detail/{vehicleId}" }, method = RequestMethod.GET)
	public List<UserDetail> getVehicleUser(@PathVariable Long vehicleId,
			@RequestParam(value = "login", required = false) String login) {
		List<UserDetail> details = new ArrayList<>();
		List<User> users = userService.getUsersForChat(vehicleId, login);
		for (User user : users) {
			details.add(getUserDetail(user));
		}
		return details;
