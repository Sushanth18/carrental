...
public interface VehicleRepository extends  PagingAndSortingRepository<Vehicle, Long>, VehicleRepositoryCustom {
	...
	@Transactional
	@Query(value="SELECT DISTINCT v FROM Vehicle v LEFT JOIN FETCH v.equipmentList WHERE v.userId =:userId ORDER BY v.brand ASC, v.model ASC",
			countQuery = "SELECT COUNT(v) FROM Vehicle v WHERE v.userId =:userId")
    public Page<Vehicle> getVehiclesByUserForPage(Pageable pageable,@Param("userId") Long userId);
