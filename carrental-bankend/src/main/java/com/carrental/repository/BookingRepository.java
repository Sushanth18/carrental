...
public interface BookingRepository extends PagingAndSortingRepository<Booking, Long>, BookingRepositoryCustom {
	...
	@Query(value = "SELECT b FROM Booking b WHERE b.userId=:userId", countQuery = "SELECT COUNT(b) FROM Booking b WHERE b.userId=:userId")
	public Page<Booking> getUserBookingsForPage(Pageable pageable, @Param("userId") Long userId);

	@Query(value = "SELECT b FROM Booking b WHERE b.userId=:userId AND b.bookingStateCode='RES'", countQuery = "SELECT COUNT(b) FROM Booking b WHERE b.userId=:userId AND b.bookingStateCode='RES'")
	public Page<Booking> getUserBookingsReservedForPage(Pageable pageable, @Param("userId") Long userId);

	@Query(value = "SELECT b FROM Booking b WHERE b.userId=:userId AND b.bookingStateCode='REN'", countQuery = "SELECT COUNT(b) FROM Booking b WHERE b.userId=:userId AND b.bookingStateCode='REN'")
	public Page<Booking> getUserBookingsRentedForPage(Pageable pageable, @Param("userId") Long userId);
